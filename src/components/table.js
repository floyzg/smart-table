...
export function initTable(settings, onAction) {
    ...
    // submit для кнопок type=submit (есть name у кнопок)
    root.container.addEventListener('submit', (e) => {
        e.preventDefault();
        // e.submitter — кнопка, вызвавшая submit (поддерживается в современных браузерах)
    ...
    const render = (data) => {
        ...
        const nextRows = data.map(item => {
            ...
            if (row.elements.date) row.elements.date.textContent = String(item.date ?? '');
            if (row.elements.customer) row.elements.customer.textContent = String(item.customer ?? '');
            if (row.elements.seller) row.elements.seller.textContent = String(item.seller ?? '');
            if (row.elements.total) row.elements.total.textContent = String(item.total ?? '');
